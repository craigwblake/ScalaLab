...
object StaticMathsEigen  extends AnyRef with scalaSci.StaticScalaSciCommonOps[scalaSci.jeigen.Mat]  with scalaSci.StaticScalaSciGlobal  {
  
  implicit final def vecToMat(x: Vec): scalaSci.jeigen.Mat = { vec2Mat(x) } // implicit conversion of a Vec to Mat
  implicit final def MatToDoubleArr(x: scalaSci.jeigen.Mat) = { x.getRow(0) }
...
final def eigvals(Mmat: Mat): RichDouble1DArray = {
  ...
  final def leig(Mmat: Mat) = {
    ...
    var leftEvecs = Array.ofDim[Double](Mmat.Nrows, Mmat.Ncols)
    var rightEvecs = Array.ofDim[Double](Mmat.Nrows, Mmat.Ncols)
