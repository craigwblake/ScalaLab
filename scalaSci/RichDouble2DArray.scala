...
// Apache Common Maths
 ...
 final def +( that:  CommonMaths.Mat ): CommonMaths.Mat = {
   ...
   val result = new CommonMaths.Mat(N, M)
   var r = 0
   while (r < N)  {
      var c = 0
      while (c < M)  {
...
final def  solve( b: RichDouble1DArray): RichDouble2DArray = {
  ...
  final def  solve(b: RichDouble2DArray): RichDouble2DArray = {
    if (b.numRows() == b.numColumns)  { // direct solve
         ...
         new RichDouble2DArray(solution)
    }
