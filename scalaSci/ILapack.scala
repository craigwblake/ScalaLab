 ...
 */
 def permMatrixFromIndex( lpiv: Array[Int]) =  {
  ...
  var  (realEvs, imEvs, leftEvecs, rightEvecs) = Eig(A)
   */
...
def  Eig(inM: Array[Array[Double]]) =   {
            ...
            val leftEvecs = evdObj.getLeftEigenvectors().toDoubleArray
            val rightEvecs = evdObj.getRightEigenvectors().toDoubleArray
    
      (realEvs, imEvs, leftEvecs, rightEvecs)
 }
...
/*
 ...
 var lefteigvecs = eigresults.leftEvecs   // left eigenvectors
 var righteigvecs = eigresults.rightEvecs  // right eigenvectors
 */
...
def MTJEig(inM: Array[Array[Double]]) =    {
        ...
        try {
          ...
          case ex: Exception => println("general exception  in MTJEig")  
             ...
             er.leftEvecs = denseMatrixToDoubleArray(evdObj.getLeftEigenvectors())
             er.rightEvecs = denseMatrixToDoubleArray(evdObj.getRightEigenvectors())
