...
class ScalalabFrame(private val p : Actor) extends JFrame {
  ...
  private[scalalab] val proc = actor {
    ...
    loop {  // actor loop
      receive {  // receive a GUITask and process it
      case GUITask(fn) =>
  ...
  guiTask {
    def mkMenuBar = {
      ...
      "Print Cells Interpreter State" does {
        ...
        new Menu("File") {
          "New" does {p ! NewFile()}
          "Open File..." does {p ! OpenFile()}
          ---
          "Save File                Ctrl+S" does {editor.save(false)}
          "Save As ..." does {editor.save(true)}
        }
        new Menu("Interpreter") {
          "Interpret               Shift+Enter" does {editor.interpret}
          "Interpret All      Ctrl+Shift+Enter" does {editor.interpretAll}
          "Restart with default libraries                     Ctrl+R" does {p ! Restart()}
          "Restart with EJML  based libraries "  does { p ! RestartEJML() }
          "Restart with MTJ  based libraries "  does { p ! RestartMTJ() }
          "Restart with Apache Common Maths  based libraries "  does { p ! RestartApacheCommons() }
          }	
        new Menu("CodeCell") {
