...
class OuterEditor(listener : Actor) extends JTextPane {
  ...
  class EditorGroup(private val out : Boolean, val text : Option[String], private var commandNum : Option[Int]) {
    def this(isOut : Boolean) = this (isOut, None, None)   // out cells do not have associatd text and command number 
  ...
  def restart {listener ! Restart()}
  def restartEJML { listener ! RestartEJML() }
  def restartMTJ  { listener ! RestartMTJ() }
