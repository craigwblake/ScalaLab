    ...
    public void keyPressed(KeyEvent e)
    {
        ...
        switch (keyValue) {
            ...
            case KeyEvent.VK_F6:
                ...
                scalaExec.Interpreter.GlobalValues.inScalaMode = true;   // F6 means that we try to execute Scala code
...
for (int k=0; k<sf.length; k++) {
  ...
  scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret( subcommand );
    ...
    break;
            ...
            case KeyEvent.VK_F8:
                scalaExec.Interpreter.GlobalValues.inScalaMode = false;   // F8 means that we try to execute Groovy code
                final String expression = getSelectedTextOrCurrentLine();
    if (e.isShiftDown()==false) {
                Object grResult = null;
                    grResult =  scalaExec.Interpreter.ControlGroovy.execWithGroovyShell(expression);
                 if (grResult != null) {
   ...
   }  // isControlDown==false and isShiftDown == false
    
            else  if (e.isControlDown() ==true) {
          
          
                Binding groovyBinding = gExec.Interpreter.GlobalValues.GroovyShell.getContext();    // get the reference to the Groovy variable binding
